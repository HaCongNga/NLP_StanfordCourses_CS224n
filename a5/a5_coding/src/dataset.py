...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print(f"dataset.py document line 172 : {document}")
        truncate_length = int(torch.randint(low=4, high=int(self.block_size * 7/8 + 1), size=(1,))[0])
        trucated_text = document[:truncate_length]
        # 2
        masked_content_len = int(np.random.normal(truncate_length/4, 1))
        prefix = trucated_text[:masked_content_len]
        masked_content = trucated_text[masked_content_len:(masked_content_len*2)]
        suffix = trucated_text[masked_content_len*2:]
        # 3
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR * (self.block_size - len(prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content))
