...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses :
        batch_parses = unfinished_parses[:batch_size] # Take the batch size
        #print(f"len batch: {len(batch_parses)} batch: {batch_parses}")
        transitions = model.predict(batch_parses)   # predicted transitions
        #dependencies = partial_parses[:batch_size].parse(transitions)   #parse batch
        #print(f"len_trans : {len(transitions)} trans : {transitions}")
        for i,sentence_parse in enumerate(batch_parses) :
            batch_parses[i].parse_step(transitions[i])
            #batch_parses[i].parse(transitions[i])
            if len(batch_parses[i].stack) == 1 and len(batch_parses[i].buffer) == 0 :
                unfinished_parses.remove(sentence_parse)
