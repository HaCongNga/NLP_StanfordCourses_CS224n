...
def naiveSoftmaxLossAndGradient(
    ...
    y_hat = softmax((np.dot(outsideVectors,centerWordVec))) # (n, )
    y_true = [0 if i != outsideWordIdx else 1 for i in range(len(y_hat))] #  (n, )
    loss = -np.log(y_hat[outsideWordIdx]) # scalar
    gradCenterVec = np.dot(outsideVectors.T, (y_hat-y_true)) # (d,1)
    gradOutsideVecs = np.outer(y_hat, centerWordVec)
    gradOutsideVecs[outsideWordIdx] = np.dot((y_hat[outsideWordIdx]-1), centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    others_vector = outsideVectors[negSampleWordIndices]
    outside_word_vec = outsideVectors[outsideWordIdx]
    o_prob_dis = np.dot(outside_word_vec.T, centerWordVec)
    o_prob = sigmoid(o_prob_dis)
    other_prob_dis = np.dot((-others_vector), centerWordVec)
   
    other_prob = np.sum(np.log(sigmoid(other_prob_dis)))
    loss = -np.log(o_prob) - other_prob 
    #gradCenterVec = -(1-o_prob)*outside_word_vec + np.dot(( 1-sigmoid(other_prob_dis)).T, others_vector)
    gradCenterVec = -(1-o_prob)*outside_word_vec + np.dot(others_vector.T, ( 1-sigmoid(other_prob_dis)))
    
    unique, indices, counts = np.unique(negSampleWordIndices, return_index=True, return_counts=True)
    gradOutsideVecs = np.zeros_like(outsideVectors)

    gradOutsideVecs[unique] = np.outer( (1-sigmoid(other_prob_dis[indices]))*counts, (centerWordVec))
    #gradOutsideVecs[unique] = ((1-sigmoid(other_prob_dis[unique]))*counts)*(centerWordVec)
    gradOutsideVecs[outsideWordIdx] = -(1-o_prob)*centerWordVec.T
    """
    print(f"other_prob_dis: {other_prob_dis}")
    print(f"other_prob_dis[unique] : {unique}, {other_prob_dis[unique]}")
    print(f"other_prob_dis[unique]_prob : {unique}, {(1-sigmoid(other_prob_dis[unique]))}")
    print(f"other_prob_dis[unique]*counts : {counts}, {unique}, {(1-sigmoid(other_prob_dis[unique]))*counts}")
    print(f"centerWordVec: {centerWordVec}")
    print(f"gradOutsideVecs: {gradOutsideVecs}")
    print(f"gradOutsideVecs[unique] : {np.outer( (1-sigmoid(other_prob_dis[unique]))*counts, (centerWordVec))}")
    """
    
    #Calculate the gradient for gradOutsideVecs
    """
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = -(1 - o_prob) * centerWordVec
    for i, neg_index in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_index] += (1 - sigmoid(other_prob_dis)[i]) * centerWordVec
        print(f"iter {i} , neg_index {neg_index}, prob {(1 - sigmoid(other_prob_dis)[i])}, centerWord: {centerWordVec}, grad: {gradOutsideVecs[neg_index]}")
    

    print(f"gradOutsideVecs right: {gradOutsideVecs}")
    print(f"{indices}")
    """
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    v_c = centerWordVectors[center_word_id] # center word vector
    #num_outside_words = 0
    for outside_word in outsideWords :
        #num_outside_words
        outsideWordIdx = word2Ind[outside_word]
        loss_o, gradCenterVector_o, gradOutsideVector_o = word2vecLossAndGradient(v_c, outsideWordIdx, outsideVectors,dataset)
        loss += loss_o
        gradCenterVecs[center_word_id] += gradCenterVector_o
        gradOutsideVectors += gradOutsideVector_o
