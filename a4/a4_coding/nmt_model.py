...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # For more efficient computation, pack_padded_sentence helps the LSTM where is the padded
        # It is also more accurate to get the last hidden state when pack_padded_sequence
        X = pack_padded_sequence(X, source_lengths)
        raw_enc_hiddens, (last_hiddens, last_cells) = self.encoder(X)
        enc_hiddens, _ = pad_packed_sequence(raw_enc_hiddens) # Travel back to the initial size
        enc_hiddens = torch.permute(enc_hiddens, (1, 0, 2))
        last_hidden = torch.cat((last_hiddens[0], last_hiddens[1]), 1)
        last_cell = torch.cat((last_cells[0], last_cells[1]), 1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj,torch.unsqueeze(dec_hidden, 2)).squeeze(2)  #(len,h) * (h,1)
